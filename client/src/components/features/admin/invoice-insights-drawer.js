// File: src/components/features/admin/invoice-insights-drawer.js
"use client";
...
const TOP_CUSTOMER_PAGE_SIZE = 10;
      ...
      <Card>
        ...
        <CardContent className="space-y-3 text-sm">
          ...
          <div className="flex justify-between gap-4">
            ...
            <span className="font-medium">{formatAmount(detail.amount ?? detail.paymentAmount, detail.currency ?? detail.paymentCurrency)}</span>
          </div>
        </CardContent>
      </Card>
      <Card>
...
export default function InvoiceInsightsDrawer() {
  ...
  const summaryQuery = useInfiniteQuery({
    ...adminInvoiceSummaryInfiniteOptions(sanitizedFilters, { byUserLimit: TOP_CUSTOMER_PAGE_SIZE }),
    enabled: open,
  });

  const invoicesQuery = useInfiniteQuery({
    ...adminInvoiceListOptions(sanitizedFilters, { limit: INVOICE_PAGE_SIZE }),
    enabled: open,
  });
  ...
  return (
    <Sheet open={open} onOpenChange={handleOpenChange}>
      ...
      <SheetContent
        side="right"
        className="flex h-full w-full max-w-3xl flex-col gap-0 sm:max-w-xl md:max-w-3xl"
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-10">
          <FiltersSection filters={filters} onChange={handleFilterChange} onReset={handleResetFilters} />
          <Separator />
          ...
          <TopCustomersSection
            ...
            isFetchingNextPage={summaryQuery.isFetchingNextPage}
